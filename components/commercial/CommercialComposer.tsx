...
export function CommercialComposer({ mode, isArabic, title, description, children, summary }: { mode: CommercialComposerMode; isArabic: boolean; title: string; description: string; children: ReactNode; summary?: ReactNode }) {
  return <div className={`grid gap-6 ${summary ? "xl:grid-cols-[minmax(0,1fr)_320px]" : "grid-cols-1"}`} dir={isArabic ? "rtl" : "ltr"} data-commercial-composer={mode}>
    ...
    {summary ? <aside className="xl:sticky xl:top-24 xl:self-start">{summary}</aside> : null}
  </div>;
}
