import { useState, type ChangeEvent, type ClipboardEvent, type DragEvent, type KeyboardEvent, type ReactNode, type RefObject } from "react";
...
type Props = {
  ...
  attachmentStatus?: "READY" | "UPLOADING" | "INSPECTING";
...
// Phase 2A-6: workbooks join the attachable set so a BOQ can be inspected by
// the governed spreadsheet pipeline. The server still verifies the container
// and the OOXML structure before anything is parsed.
const MIME_TYPES = new Set([
  ...
  // Phase 2A-7: DXF has no single registered MIME type, so every type a browser
  // may report is accepted here. The real decision is made from the bytes
  // downstream: a DWG renamed to .dxf is rejected by the upload gate and named
  // as a DWG rather than being read as a drawing.
  "image/vnd.dxf",
  "application/dxf",
  "application/x-dxf",
  "image/x-dxf",
  ...
  "application/x-ifc",
  "model/vnd.ifc",
  // Phase 2A-9: proprietary ORIGINALS join the attachable set so they can
  // enter the governed derivation workflow. VOKA does not parse them: the
  // composer must never say a DWG/RVT was inspected, and the bytes still
  ...
  "application/dwg",
  "application/x-dwg",
  "application/vnd.revit.rvt",
  "application/x-revit-rvt",
  "application/vnd.autodesk.revit.rvt",
  "application/revit",
]);
const EXTENSIONS = /\.(?:pdf|png|jpe?g|webp|xlsx|dxf|ifc|dwg|rvt)$/iu;
...
/** A Revit family definition is truthfully unsupported — never accepted, never vague. */
const RFA_EXTENSION = /\.rfa$/iu;

export function Composer({ isArabic, value, inputRef, attachment, attachmentStatus = "READY", primaryActionLabel, hasText, isListening, disabled, voiceUnavailable, interimTranscript, onChange, onKeyDown, onPrimaryAction, onAttachment, onRemoveAttachment, controls, status, elevated = false }: Props) {
  const [attachmentError, setAttachmentError] = useState<string | null>(null);
  const acceptFile = (file: File | null) => {
    if (!file) return;
    const declaredType = file.type.toLowerCase();
    if (RFA_EXTENSION.test(file.name)) {
      setAttachmentError(isArabic
        ? "هذا تعريف عائلة Revit‏ (.rfa)، وهو غير مدعوم كملف مشروع/نموذج في هذه المرحلة. صدّر المشروع الذي يستخدمه كملف IFC وأرفق النموذج المُصدَّر."
    ...
    if (!validType) { setAttachmentError(isArabic ? "يسمح بملفات PDF أو PNG أو JPG أو WebP أو XLSX أو DXF أو IFC أو DWG أو RVT فقط." : "Only PDF, PNG, JPG, WebP, XLSX, DXF, IFC, DWG, and RVT files are supported."); return; }
  ...
  return <div className={`relative z-20 min-w-0 ${elevated ? "sticky bottom-3" : ""}`} onPaste={handlePaste} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
    ...
    <div data-testid="commercial-composer-input" className="group relative overflow-hidden rounded-[1.75rem] border border-white/[0.11] bg-slate-900/90 p-2 shadow-[0_28px_90px_-34px_rgba(14,165,233,.35),0_18px_60px_-36px_rgba(99,102,241,.45),inset_0_1px_0_rgba(255,255,255,.055)] backdrop-blur-2xl transition duration-300 focus-within:border-sky-300/40 focus-within:shadow-[0_30px_100px_-32px_rgba(14,165,233,.48),0_20px_70px_-34px_rgba(99,102,241,.5),inset_0_1px_0_rgba(255,255,255,.08)] motion-reduce:transition-none sm:p-3">
      ...
      {attachment ? <div className="mb-2 flex min-w-0 items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.035] px-3 py-2.5 text-xs"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-sky-300/10 text-sky-200"><AssistantIcon name="attach" className="h-4 w-4" /></span><span className="min-w-0 truncate text-slate-300">{attachment.name}</span><span data-testid="attachment-upload-state" role="status" className="hidden shrink-0 text-emerald-300 sm:inline">{attachmentStatus === "UPLOADING" ? (isArabic ? "جارٍ الرفع…" : "Uploading…") : attachmentStatus === "INSPECTING" ? (isArabic ? "جارٍ الفحص…" : "Inspecting…") : (isArabic ? "جاهز للإرسال" : "Ready to upload")}</span><button type="button" onClick={() => { setAttachmentError(null); onRemoveAttachment(); }} className="ms-auto shrink-0 rounded-lg px-2 py-1 text-rose-300 outline-none hover:bg-rose-400/10 focus-visible:ring-2 focus-visible:ring-sky-400">{isArabic ? "إزالة" : "Remove"}</button></div> : null}
      ...
      {attachmentError ? <p role="alert" className="px-3 pb-1 text-xs text-rose-300">{attachmentError}</p> : null}
