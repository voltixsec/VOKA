...
describe("UniversalLibraryBatchWizard", () => {
  it("shows a refresh failure instead of leaving an unhandled rejection", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("journey unavailable")));
    render(<UniversalLibraryBatchWizard sourceId="s" batchExternalKey="b" sourceNamespace="n" />);
    await screen.findByText("journey unavailable");
    fireEvent.click(screen.getByText("Refresh journey"));
    await waitFor(() => expect(fetch).toHaveBeenCalledTimes(2));
    expect(screen.getByTestId("ucl-batch-wizard-error")).toHaveTextContent("journey unavailable");
  });
  afterEach(() => {
    vi.unstubAllGlobals();
  });
