  ...
  public calculate(rawInputs: Record<string, any>): SystemCalculationResult {
    ...
    const components: SystemComponent[] = [
      {
        ...
        formulaExplanation: `Math.ceil((${effectiveArea} m² / ${boardAreaM2} m²) * ${layersCount} layer(s) * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى ((${effectiveArea} م² ÷ ${boardAreaM2} م²) × ${layersCount} طبقة × ${wasteMultiplier} معامل الهدر)`,
        category: "MATERIALS",
      },
      {
        componentKey: "STUD_FRAMING",
        name: `قطاعات معدنية رأسية (C-Studs ${studSpacingCm} سم)`,
        nameAr: `قطاعات معدنية رأسية (${studSpacingCm} سم)`,
        nameEn: `Metal C-Stud Framing (${studSpacingCm} cm spacing)`,
        itemType: "PRODUCT",
        quantity: totalStudMeters,
        unit: "LM",
        provenance: "CALCULATED",
        formulaExplanation: `Math.ceil(${effectiveArea} m² * ${linearStudsPerM2.toFixed(2)} m/m² * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى (${effectiveArea} م² × ${linearStudsPerM2.toFixed(2)} م/م² × ${wasteMultiplier} معامل الهدر)`,
        category: "MATERIALS",
      },
      {
        componentKey: "RUNNER_TRACKS",
        name: "قطاعات معدنية أفقية (U-Runner Tracks)",
        nameAr: "قطاعات معدنية أفقية",
        nameEn: "Metal U-Runner Tracks",
        itemType: "PRODUCT",
        quantity: totalTrackMeters,
        unit: "LM",
        provenance: "CALCULATED",
        formulaExplanation: `Math.ceil(${effectiveArea} m² * 0.85 m/m² * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى (${effectiveArea} م² × 0.85 م/م² × ${wasteMultiplier} معامل الهدر)`,
        category: "MATERIALS",
      },
      {
        componentKey: "DRYWALL_SCREWS",
        name: "براغي تثبيت أجهزة وجبس بورد (Drywall Screws)",
        nameAr: "براغي تثبيت أجهزة وجبس بورد",
        nameEn: "Drywall Screws",
        itemType: "PRODUCT",
        quantity: totalScrews,
        unit: "Pcs",
        provenance: "CALCULATED",
        formulaExplanation: `Math.ceil(${effectiveArea} m² * 18 screws/m² * ${layersCount} layer(s) * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى (${effectiveArea} م² × 18 برغي/م² × ${layersCount} طبقة × ${wasteMultiplier} معامل الهدر)`,
        category: "ACCESSORIES",
      },
      {
        componentKey: "JOINT_TAPE",
        name: "شريط فواصل ورقي/فيبر (Joint Tape)",
        nameAr: "شريط فواصل ورقي/فيبر",
        nameEn: "Joint Fiber/Paper Tape",
        itemType: "PRODUCT",
        quantity: totalTapeMeters,
        unit: "LM",
        provenance: "CALCULATED",
        formulaExplanation: `Math.ceil(${effectiveArea} m² * 1.6 m/m² * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى (${effectiveArea} م² × 1.6 م/م² × ${wasteMultiplier} معامل الهدر)`,
        category: "ACCESSORIES",
      },
      {
        componentKey: "JOINT_COMPOUND",
        name: "معجون فواصل جبس بورد (Joint Compound)",
        nameAr: "معجون فواصل جبس بورد",
        nameEn: "Joint Compound / Putty",
        itemType: "PRODUCT",
        quantity: totalCompoundKg,
        unit: "KG",
        provenance: "CALCULATED",
        formulaExplanation: `Math.ceil(${effectiveArea} m² * 1.2 kg/m² * ${layersCount} layer(s) * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى (${effectiveArea} م² × 1.2 كجم/م² × ${layersCount} طبقة × ${wasteMultiplier} معامل الهدر)`,
        category: "ACCESSORIES",
      },
      {
        ...
        unit: "Pair",
        provenance: "CALCULATED",
        formulaExplanation: `Math.ceil(${effectiveArea} m² * 3 pairs/m² * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى (${effectiveArea} م² × 3 زوج/م² × ${wasteMultiplier} معامل الهدر)`,
        category: "ACCESSORIES",
      },
    ];
    ...
    if (includeInsulation) {
      ...
      components.push({
        ...
        unit: "m²",
        provenance: "CALCULATED",
        formulaExplanation: `Math.ceil(${effectiveArea} m² * ${wasteMultiplier} wastage)`, formulaExplanationAr: `تقريب لأعلى (${effectiveArea} م² × ${wasteMultiplier} معامل الهدر)`,
        category: "MATERIALS",
      });
    }
    ...
    if (includeInstallation) components.push({
      ...
      unit: "m²",
      provenance: "CALCULATED",
      formulaExplanation: `Installation & finishing labor for ${effectiveArea} m²`, formulaExplanationAr: `مصنعية التركيب والتشطيب لمساحة ${effectiveArea} م²`,
      category: "SERVICES",
    });
