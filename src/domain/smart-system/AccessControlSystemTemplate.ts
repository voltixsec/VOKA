...
export class AccessControlSystemTemplate implements ISystemTemplate {
  ...
  calculate(raw: Record<string, any>): SystemCalculationResult {
    ...
    const components: SystemComponent[] = [
      { componentKey: "ACCESS_CONTROLLERS", name: "لوحات تحكم دخول (سعة 4 أبواب)", nameAr: "لوحات تحكم دخول (سعة 4 أبواب)", nameEn: "Access Control Panels (4-door capacity)", itemType: "PRODUCT", quantity: panels, unit: "Unit", provenance: "CALCULATED", formulaExplanation: `Math.ceil(${doors} doors / 4) = ${panels} panel(s)`, formulaExplanationAr: `تقريب لأعلى (${doors} باب ÷ 4) = ${panels} لوحة`, category: "HARDWARE" },
      { componentKey: "ACCESS_READERS", name: "قارئات دخول", nameAr: "قارئات دخول", nameEn: "Access Control Readers", itemType: "PRODUCT", quantity: readers, unit: "Unit", provenance: "CALCULATED", formulaExplanation: `${doors} doors × ${accessDirection === "ENTRY_EXIT" ? 2 : 1} controlled side(s) = ${readers} reader(s)`, formulaExplanationAr: `${doors} باب × ${accessDirection === "ENTRY_EXIT" ? 2 : 1} جهة تحكم = ${readers} قارئ`, category: "HARDWARE" },
      { componentKey: "ELECTRIC_LOCKS", name: "أقفال كهربائية", nameAr: "أقفال كهربائية", nameEn: "Electric Door Locks", itemType: "PRODUCT", quantity: doors, unit: "Unit", provenance: "CALCULATED", formulaExplanation: `1 lock per controlled door = ${doors}`, formulaExplanationAr: `قفل واحد (1) لكل باب يتم التحكم به = ${doors}`, category: "HARDWARE" },
      { componentKey: "DOOR_CONTACTS", name: "حساسات حالة الباب", nameAr: "حساسات حالة الباب", nameEn: "Door Position Contacts", itemType: "PRODUCT", quantity: doors, unit: "Unit", provenance: "CALCULATED", formulaExplanation: `1 contact per controlled door = ${doors}`, formulaExplanationAr: `حساس واحد (1) لكل باب يتم التحكم به = ${doors}`, category: "HARDWARE" },
      ...(accessDirection === "ENTRY_ONLY" ? [{ componentKey: "EXIT_BUTTONS", name: "أزرار خروج", nameAr: "أزرار خروج", nameEn: "Request-to-Exit Buttons", itemType: "PRODUCT" as const, quantity: doors, unit: "Unit", provenance: "CALCULATED" as const, formulaExplanation: `1 exit button per entry-only door = ${doors}`, formulaExplanationAr: `زر خروج واحد (1) لكل باب يتحكم بالدخول فقط = ${doors}`, category: "HARDWARE" }] : []),
      ...(includeInstallation ? [
        { componentKey: "ACCESS_CABLE", name: "كابل نظام التحكم بالدخول", nameAr: "كابل نظام التحكم بالدخول", nameEn: "Access Control System Cable", itemType: "PRODUCT" as const, quantity: doors * cableMetersPerDoor!, unit: "m", provenance: "CALCULATED" as const, formulaExplanation: `${doors} doors × ${cableMetersPerDoor} m = ${doors * cableMetersPerDoor!} m`, formulaExplanationAr: `${doors} باب × ${cableMetersPerDoor} متر = ${doors * cableMetersPerDoor!} متر`, category: "INFRASTRUCTURE" },
        { componentKey: "ACCESS_INSTALLATION", name: "تركيب وبرمجة واختبار نقاط الدخول", nameAr: "تركيب وبرمجة واختبار نقاط الدخول", nameEn: "Access Point Installation, Programming & Testing", itemType: "SERVICE" as const, quantity: doors, unit: "Point", provenance: "CALCULATED" as const, formulaExplanation: `1 installation point per controlled door = ${doors}`, formulaExplanationAr: `نقطة تركيب واحدة (1) لكل باب يتم التحكم به = ${doors}`, category: "SERVICE" },
      ] : []),
    ];
