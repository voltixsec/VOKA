...
describe("Conversation solution approval state sync", () => {
  ...
  it("promotes approved ceramic estimates and rebuilds the exact governed workspace and draft state", () => {
    ...
    const draft = adaptCommercialHandoffToQuotationDraft({ companyId: "company-1", handoff: { runtimeId: "runtime-1", confirmedFacts: { ...approved.confirmed, "customer.name": explicitFact("customer.name", "National Telecom") }, commercialLines: handoffLines, toolEvidence: [], createdAt: "2026-08-31T18:00:00.000Z" }, customer: { status: "RESOLVED", id: "customer-1", name: "National Telecom" }, defaults: { currencyCode: "KWD", termsAr: null, termsEn: null, payment: null, delivery: null, warranty: null, validity: null }, locale: "en" });
