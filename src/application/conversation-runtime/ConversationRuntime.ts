...
export class ConversationRuntime {
  ...
  async execute(input: ConversationTurnInput): Promise<ConversationRuntimeState> {
    ...
    if (vehicleIdentity || base.workspace?.engineering.system?.key === "VEHICLE_ELEVATOR") turnFacts = { ...turnFacts, ...detectExplicitVehicleElevatorFacts(message, this.now(), Boolean(base.confirmedFacts["system.quantity"] || base.confirmedFacts["system.numberOfStops"])) };
