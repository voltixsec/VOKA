  ...
  async execute(input: Parameters<ConversationToolPort["execute"]>[0]): Promise<ToolObservation> {
    ...
    if (["ATTACHMENT_INSPECTION", "DRAWING_INSPECTION", "BOQ_INSPECTION"].includes(input.request.kind)) {
      ...
      return this.inspection.inspect({
        ...
        artifactId: input.request.attachmentId!,
        kind: input.request.kind as "ATTACHMENT_INSPECTION" | "DRAWING_INSPECTION" | "BOQ_INSPECTION",
        query: input.request.query,
        locale: input.locale,
        governedFacts: input.graph.requirements.map(({ key, value, provenance }) => ({ key, value, provenance })),
      });
    }
