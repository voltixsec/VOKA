...
export class PrismaSourceArtifactInspection implements SourceArtifactInspectionPort {
  async inspect(input: Parameters<SourceArtifactInspectionPort["inspect"]>[0]): Promise<ToolObservation> {
    ...
    let extractedPages = Array.isArray(artifact.extractedPages)
      ? artifact.extractedPages.flatMap((page: unknown) => {
          if (!page || typeof page !== "object") return [];
          const value = page as { pageNumber?: unknown; text?: unknown };
          return typeof value.pageNumber === "number" && typeof value.text === "string" ? [{ pageNumber: value.pageNumber, text: value.text }] : [];
        })
      : [];
    if (!extractedText) {
      try {
