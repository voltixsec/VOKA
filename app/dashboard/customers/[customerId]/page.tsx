...
type Customer = Record<string, string | number | null> & { id: string; code: string; name: string; status: string };
...
export default function CustomerDetailPage() {
  ...
  return <section className="space-y-6" dir={isArabic ? 'rtl' : 'ltr'}>
    <SectionHeader eyebrow={customer.code} title={customer.name} description={isArabic ? 'بيانات العميل الحالية والمؤكدة.' : 'Current canonical customer details.'} actions={<div className="flex gap-3"><Link className="rounded-xl border border-white/10 px-4 py-2 text-sm text-slate-200 hover:bg-white/5" href="/dashboard/customers">{isArabic ? 'العملاء' : 'Customers'}</Link><Link className="rounded-xl bg-sky-400 px-4 py-2 text-sm font-semibold text-slate-950" href={`/dashboard/customers/${encodeURIComponent(customer.id)}/edit`}>{isArabic ? 'تعديل العميل' : 'Edit customer'}</Link></div>} />
