...
function toForm(customer: Record<string, unknown>): CustomerFormValue {
  ...
  return { ...emptyCustomerForm, code: text('code'), name: text('name'), nameAr: text('nameAr'), nameEn: text('nameEn'), type: customer.type === 'INDIVIDUAL' ? 'INDIVIDUAL' : 'COMPANY', status: ['LEAD','ACTIVE','INACTIVE','BLOCKED'].includes(String(customer.status)) ? customer.status as CustomerFormValue['status'] : 'LEAD', legalName: text('legalName'), email: text('email'), phone: text('phone'), mobile: text('mobile'), whatsapp: text('whatsapp'), taxNumber: text('taxNumber'), addressLine1: text('addressLine1'), addressLine2: text('addressLine2'), city: text('city'), state: text('state'), postalCode: text('postalCode'), countryCode: text('countryCode') || 'KW', preferredLocale: customer.preferredLocale === 'AR' || customer.preferredLocale === 'EN' ? customer.preferredLocale : '', preferredCurrency: text('preferredCurrency'), creditLimit: text('creditLimit'), paymentTermDays: text('paymentTermDays'), notes: text('notes') };
}
