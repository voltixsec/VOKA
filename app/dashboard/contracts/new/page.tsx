...
export default function NewContractPage() {
  ...
  const [contractDate, setContractDate] = useState(todayDateInput());
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const [custRes, itemRes, taxRes, unitRes, compRes] = await Promise.allSettled([
        ...
        if (compRes.status === "fulfilled" && compRes.value.ok) {
          ...
          const curr = compJson.data?.defaultCurrency;
          if (!cancelled && typeof curr === "string" && /^[A-Z]{3}$/.test(curr)) {
    ...
    try {
      ...
      const payload = {
        ...
        lines: lines.map((l, idx) => ({
          ...
          discountType: l.taxRateId ? null : undefined,
        })),
      ...
      if (!res.ok) throw new Error(json?.error?.message || t("تعذر إنشاء العقد", "Failed to create contract"));
  ...
  return (
    <section className="space-y-6" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <SectionHeader
        eyebrow={t("عقد جديد", "New contract")}
        title={t("منشئ العقد التجاري", "Contract Builder")}
        description={t(
          "أنشئ عقدًا تجاريًا محدداً بالعميل والبنود ومراحل الدفع.",
          "Build a commercial contract with lines, pricing, and payment milestones.",
        )}
      />

      <form onSubmit={submit}><CommercialComposer mode="CONTRACT" isArabic={isArabic} title={t("مسودة عقد جديدة", "New contract draft")} description={t("محرر تجاري موحد مع أقسام العقد ومراحل الدفع الخاصة به.", "The shared commercial editor with contract-specific terms and payment milestones.")}>
        {error && (
        ...
        <Card>
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5">
            ...
            <div className="flex gap-3">
              <Button type="button" variant="secondary" onClick={() => router.push("/dashboard/contracts")}>
                {t("إلغاء", "Cancel")}
              </Button>
              <Button type="submit" disabled={saving || !customerId || lines.length === 0}>
                {saving ? t("جارٍ الحفظ...", "Saving...") : t("إنشاء العقد", "Create contract")}
              </Button>
            </div>
          </div>
        </Card>
