...
import { CommercialComposer, CommercialLineEditor, ComposerSection, type CommercialCatalogItem, type CommercialLineDraft } from "@/components/commercial";
...
export default function EditInvoicePage() {
  ...
  useEffect(() => { let cancelled = false; void (async () => { try {
    ...
    setCustomers(customerBody.data?.customers ?? []); setItems((Array.isArray(itemBody.data) ? itemBody.data : []).map((item: any) => ({ id: item.id, name: item.name, code: item.code, type: item.type, salePrice: Number(item.salePrice ?? 0), description: item.description, unitName: item.unit?.symbol ?? item.unitName ?? "PCS", taxPercentage: Number(item.taxRate?.percentage ?? 0) })));
  } catch (caught) { setError(caught instanceof Error ? caught.message : "Load failed"); } finally { if (!cancelled) setLoading(false); } })(); return () => { cancelled = true; }; }, [invoiceId, isArabic]);
