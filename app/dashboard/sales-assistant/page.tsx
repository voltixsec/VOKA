      ...
      <div data-testid="commercial-composer" data-commercial-state={commercialPhase(workingDraft, isGenerating, resultStale)} className="flex min-w-0 flex-col gap-2 rounded-2xl border border-white/10 bg-slate-900/70 p-3 shadow-soft sm:p-4">
        ...
        {workingDraft && !resultStale && (
          <div className="min-w-0 space-y-2 break-words rounded-xl border border-sky-400/20 bg-slate-950/80 p-3" data-testid="commercial-conversation">
            ...
            {workingDraft.canonicalProposal?.agenticState?.provisionalSystem && <p className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-2 text-xs text-amber-200" data-testid="provisional-system-review">{workingDraft.canonicalProposal.agenticState.systemName} · {isArabic ? "نموذج بحثي مؤقت — يتطلب تحققاً هندسياً ومراجعة بشرية." : "Provisional researched model — engineering verification and human review required."}</p>}
