  ...
  return (
    <div className={`space-y-4 max-w-5xl min-w-0 ${isArabic ? "font-[var(--font-cairo)]" : ""}`} dir={isArabic ? "rtl" : "ltr"}>
      ...
      <div data-testid="commercial-composer" data-commercial-state={commercialPhase(workingDraft, isGenerating, resultStale)} className="flex min-w-0 flex-col gap-2 rounded-2xl border border-white/10 bg-slate-900/70 p-3 shadow-soft sm:p-4">
        ...
        {(workingDraft?.conversationMessages?.length || workingDraft?.turns.length) ? (
          <section data-testid="conversation-timeline" aria-label={isArabic ? "المحادثة" : "Conversation"} className="max-h-[34rem] space-y-3 overflow-y-auto rounded-2xl border border-white/10 bg-slate-950/70 p-3 sm:p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-300">{isArabic ? "المحادثة" : "Conversation"}</p>
            {(workingDraft.conversationMessages ?? workingDraft.turns).map((turn, index) => {
              const user = (turn.role ?? "USER") === "USER";
              return <div key={`${index}-${turn.text.slice(0, 20)}`} className={`flex ${user ? (isArabic ? "justify-start" : "justify-end") : (isArabic ? "justify-end" : "justify-start")}`}>
                <div className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-7 ${user ? "bg-sky-400 text-slate-950" : "border border-white/10 bg-slate-900 text-slate-100"}`}>
                  <p className="mb-1 text-[10px] font-bold uppercase tracking-wider opacity-70">{user ? (isArabic ? "أنت" : "You") : "VOKA"}</p>
