...
vi.mock("@/lib/api", () => ({
  ApiError: class ApiError extends Error { static badRequest(code: string, message: string) { return Object.assign(new Error(message), { code, status: 400 }); } static notFound(code: string, message: string) { return Object.assign(new Error(message), { code, status: 404 }); } },
...
describe("source artifact intake", () => {
  ...
  it("does not resolve an artifact belonging to another tenant", async () => {
    findFirst.mockResolvedValueOnce(null);
    await expect(GET(new Request("https://voka.local/api/source-artifacts/artifact-from-other-tenant"))).rejects.toMatchObject({ code: "SOURCE_ARTIFACT_NOT_FOUND", status: 404 });
    expect(findFirst).toHaveBeenCalledWith(expect.objectContaining({ where: { id: "artifact-from-other-tenant", companyId: "tenant-1" } }));
  });
});
