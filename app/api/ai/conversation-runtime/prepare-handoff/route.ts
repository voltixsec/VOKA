...
const TRUSTED = new Set(["USER_EXPLICIT", "USER_CORRECTION", "VERIFIED_DOCUMENT", "VERIFIED_DATABASE", "TRUSTED_PROFILE", "DETERMINISTIC_DERIVATION"]);
...
export const POST = withCompanyAuth(["OWNER", "ADMIN", "SALES"], async (request, _auth, company) => {
  ...
  const confirmedFacts = Object.fromEntries(Object.entries(state.confirmedFacts).filter((entry): entry is [string, ConfirmedFact] => {
    ...
    if (fact.provenance === "USER_EXPLICIT" || fact.provenance === "USER_CORRECTION") return Boolean(fact.evidence?.trim()) && userText.some((message) => message.includes(fact.evidence.normalize("NFKC").toLocaleLowerCase()));
    return true;
  }));
  ...
  const handoff: CommercialSolutionHandoff = { runtimeId: state.runtimeId, confirmedFacts, commercialLines, toolEvidence: state.toolResults.filter((result) => result.status === "COMPLETED"), createdAt: new Date().toISOString() };
