...
import { serializeUniversalBrand } from "../serialize-universal";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const repository = new PrismaUniversalLibraryRepository(prisma);
const searchUniversalBrands = new SearchUniversalBrands(repository);
...
export const GET = withCompanyAuth(
  ...
  async (request: Request) => {
    ...
    const brands = await searchUniversalBrands.execute({
      query,
      manufacturerId,
      isActive,
      limit,
    ...
    return apiSuccess(brands.map(serializeUniversalBrand), {
      headers: {
